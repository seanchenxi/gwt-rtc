 ...
 * User: Xi
 */
public class ProgressEvent<E extends NativeEvent> extends FnEvent<E, ProgressEvent.Handler<E>> {

    public interface Handler<E extends NativeEvent> extends EventHandler {
        void onProgress(ProgressEvent<E> event);
    }
    ...
    public static <E extends NativeEvent> Type<Handler<E>> getType() {
        if (TYPE == null) {
            TYPE = new Type<Handler<NativeEvent>>(EventType.PROGRESS, new ProgressEvent<NativeEvent>());
        }
        return (Type)TYPE;
    }
