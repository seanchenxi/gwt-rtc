 ...
 * User: Xi
 */
public class LoadStartEvent<E extends NativeEvent> extends FnEvent<E, LoadStartEvent.Handler<E>> {

    public interface Handler<E extends NativeEvent> extends EventHandler {
        void onLoadStart(LoadStartEvent<E> event);
    }
    ...
    public static <E extends NativeEvent> Type<Handler<E>> getType() {
        if (TYPE == null) {
            TYPE = new Type<Handler<NativeEvent>>(EventType.LOAD_START, new LoadStartEvent<NativeEvent>());
        }
        return (Type)TYPE;
    }
